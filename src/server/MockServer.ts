...
export class MockServer {
	...
	constructor(listenPort: number, context: vscode.ExtensionContext) {
		...
		this.mockSwichButton = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
	...
	start() {
		...
		this.server
			.listen(this.listenPort, () => {
				vscode.window.showInformationMessage(`Mock 代理服务器已启动，监听端口 ${this.listenPort} 
	...
	private forwardRequest(req: http.IncomingMessage, res: http.ServerResponse, url: URL) {
	...
	private handleServerError(err: NodeJS.ErrnoException) {
		if (err.code === 'EADDRINUSE') {
			vscode.window.showErrorMessage(`端口 ${this.listenPort} 已被占用，请关闭其他占用该端口的程序后重试。`);
		} else {
			vscode.window.showErrorMessage(`启动代理服务器时发生错误: ${err.message}`);
		}
	}
